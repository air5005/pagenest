...
/* options values */
char *drmpid[VOUCHERS_COUNT_MAX];
char *drmserial[VOUCHERS_COUNT_MAX];
...
static int do_encrypt(MOBIData *m, bool use_kf8) {
    ...
    for (size_t i = 0; i < serial_count; i++) {
        mobi_ret = mobi_drm_addvoucher(m, drmserial[i], valid_from, valid_to, tags, tags_count);
        if (mobi_ret != MOBI_SUCCESS) {
...
static int do_decrypt(MOBIData *m) {
    ...
    if (pid_count) {
        for (size_t i = 0; i < pid_count; i++) {
            if (set_decryption_pid(m, drmpid[i]) == SUCCESS) {
                has_key = true;
                break;
            }
        }
    ...
    if (!has_key && serial_count) {
        for (size_t i = 0; i < serial_count; i++) {
            if (set_decryption_serial(m, drmserial[i]) == SUCCESS) {
                break;
            }
        }
    }
