...
static void mobi_drmpid_checksum(char checksum[2], const unsigned char *pid) {
    ...
    uint32_t crc = (uint32_t) ~mz_crc32(0xffffffff, pid, PIDSIZE - 2);
    crc ^= (crc >> 16);
...
static MOBI_RET mobi_drmpid_from_serial(char pid[PIDSIZE + 1], const char *serial) {
    ...
    uint32_t crc = (uint32_t) ~mz_crc32(0xffffffff, (const unsigned char *) serial, (unsigned int) serial_length);
